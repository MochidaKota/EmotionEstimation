    ...
    for center_ind in range(centers.shape[0]):
        ...
        for h in range(int(start_h), int(end_h)+1):
            for w in range(int(start_w), int(end_w)+1):
                map[h,w]=max(1 - (abs(h - AU_center_y) + abs(w - AU_center_x)) * fill_coeff / (map_size*spatial_ratio), map[h,w])

        

class AlignNet(nn.Module):
    def __init__(self, crop_size, map_size, au_num, land_num, input_dim, fill_coeff, unit_dim=8,
                 spatial_ratio=0.14):
    ...
    def forward(self, x):
        ...
        '''
        col_mean = align_output.nanmean(dim=0)
        if torch.all(~align_output.isnan()) == False:
            for j in range(align_output.size(1)):
                align_output[:,j] = torch.where(align_output[:,j].isnan(), col_mean[j], align_output[:,j])
        '''        
...
class AUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        au_output = au_output.view(au_output.size(0), 2, int(au_output.size(1)/2))
        au_output = F.softmax(au_output, dim=1)
        return au_output
    
class AUNet_0(nn.Module):
    def __init__(self, input_dim=12000, unit_dim=8):
        super(AUNet_0, self).__init__()

        self.au0_output = nn.Sequential(
            nn.Linear(input_dim, unit_dim * 64)
        )

    def forward(self, x):
        au0_output = self.au0_output(x)
        return au0_output
    
class AUNet_1(nn.Module):
    def __init__(self, au_num, input_dim=512):
        super(AUNet_1, self).__init__()

        self.au1_output = nn.Sequential(
            nn.Linear(input_dim, au_num * 2)
        )

    def forward(self, x):
        au1_output = self.au1_output(x)
        au1_output = au1_output.view(au1_output.size(0), 2, int(au1_output.size(1)/2))
        au1_output = F.softmax(au1_output, dim=1)

        return au1_output

class EmotionNet(nn.Module):
    def __init__(self, emo_num, input_dim=512):
        super(EmotionNet, self).__init__()

        self.emotion_output = nn.Sequential(
            nn.Linear(input_dim, emo_num)
        )
...
class EmotionNet_AUNetMirror(nn.Module):
    def __init__(self, emo_num, input_dim=12000, unit_dim=8):
        super(EmotionNet_AUNetMirror, self).__init__()

        self.emotion_output = nn.Sequential(
            nn.Linear(input_dim, unit_dim * 64),
            nn.Linear(unit_dim * 64, emo_num)
        )
...
class EmotionNetv2(nn.Module):
    def __init__(self, emo_num, input_dim=12000, unit_dim=8):
        super(EmotionNetv2, self).__init__()

        self.emotion_output = nn.Sequential(
            nn.Linear(input_dim, unit_dim * 256),
            nn.ReLU(inplace=True),
            nn.Dropout(0.3),
            nn.Linear(unit_dim * 256, unit_dim * 64),
            nn.ReLU(inplace=True),
            nn.Dropout(0.3),
            nn.Linear(unit_dim * 64, emo_num)
        )
...
class EmotionNetv3(nn.Module):
    def __init__(self, emo_num, input_dim=480) -> None:
        super().__init__()
        # input: 480 x 5 x 5
        self.conv1 = nn.Conv2d(input_dim, 128, kernel_size=2, stride=1)
        self.bn1 = nn.BatchNorm2d(128)
        self.relu = nn.ReLU(inplace=True)
        # output: 128 x 4 x 4
        
        self.fc = nn.Linear(2048, emo_num)
...
class EmotionNetv4(nn.Module):
    def __init__(self, emo_num, input_dim=480) -> None:
        ...
        self.fc = nn.Linear(input_dim, emo_num)
...
network_dict = {'HLFeatExtractor':HLFeatExtractor, 'HMRegionLearning':HMRegionLearning,
                ...
                'AUNet_0': AUNet_0, 'AUNet_1': AUNet_1, 'EmotionNet': EmotionNet, 'EmotionNet_AUNetMirror': EmotionNet_AUNetMirror,
                'EmotionNetv2': EmotionNetv2, 'EmotionNetv3': EmotionNetv3, 'EmotionNetv4': EmotionNetv4
                }
