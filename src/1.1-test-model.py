"""
* This script is used to test the model that was trained in '1.1-train-model.py'.    
"""
...
def main(config):
    ...
    # setting feat_list
    au_feats_df = pd.read_pickle(config.au_feats_path)
    gaze_feats_df = pd.read_pickle(config.gaze_feats_path)
    hp_feats_df = pd.read_pickle(config.hp_feats_path)
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(test_dataloader)):
            ...
            if use_feat_list == {'AU':1, 'Gaze':0, 'HP':0}:
                ...
                hp_imgs, img_paths, emotions = batch[0]
            ...
            if use_feat_list['AU'] == 1:
                ...
                stored_au_feats = au_feats_df[au_feats_df['img_path'] == img_paths].iloc[:, 1:].values
                
                if torch.all(torch.eq(au_feats, torch.tensor(stored_au_feats, dtype=torch.float).to(device))) == False:
                    img_path_temp_au += img_paths
                
            if use_feat_list['Gaze'] == 1:
                _, _, gaze_feats = gaze_feat_extractor(gaze_imgs)
                
                stored_gaze_feats = gaze_feats_df[gaze_feats_df['img_path'] == img_paths].iloc[:, 1:].values
                if torch.all(torch.eq(gaze_feats, torch.tensor(stored_gaze_feats, dtype=torch.float).to(device))) == False:
                    img_path_temp_gaze += img_paths
            
            if use_feat_list['HP'] == 1:
                _, hp_feats = hp_feat_extractor(hp_imgs)
                
                stored_hp_feats = hp_feats_df[hp_feats_df['img_path'] == img_paths].iloc[:, 1:].values
                if torch.all(torch.eq(hp_feats, torch.tensor(stored_hp_feats, dtype=torch.float).to(device))) == False:
                    img_path_temp_hp += img_paths
    ...
    if config.save_res == True:
        ...
        # save img_path_temp
        if use_feat_list['AU'] == 1:
            img_path_temp_au_df = pd.DataFrame(img_path_temp_au, columns=["img_path"])
            img_path_temp_au_df.to_csv(save_res_dir + "/" + f"{config.target_emo}_img_path_temp_au.csv", index=False)
            print(f"diff au: {len(img_path_temp_au)}")
            
        if use_feat_list['Gaze'] == 1:
            img_path_temp_gaze_df = pd.DataFrame(img_path_temp_gaze, columns=["img_path"])
            img_path_temp_gaze_df.to_csv(save_res_dir + "/" + f"{config.target_emo}_img_path_temp_gaze.csv", index=False)
            print(f"diff gaze: {len(img_path_temp_gaze)}")
            
        if use_feat_list['HP'] == 1:
            img_path_temp_hp_df = pd.DataFrame(img_path_temp_hp, columns=["img_path"])
            img_path_temp_hp_df.to_csv(save_res_dir + "/" + f"{config.target_emo}_img_path_temp_hp.csv", index=False)
            print(f"diff hp: {len(img_path_temp_hp)}")
