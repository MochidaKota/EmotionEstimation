"""
* This script is used to test the model that was trained in '2.1-train-model.py'.    
"""
...
def main(config):
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(test_dataloader)):
            ...
            count = 0
            if use_feat_list['AU'] == 1:
                au_seq_feats, img_paths, emotions = batch[count]
            ...
            if use_feat_list['Gaze'] == 1:
                if count == 0:
                    gaze_seq_feats, img_paths, emotions = batch[count]
                else:
            ...
            if use_feat_list['HP'] == 1:
                if count == 0:
                    hp_seq_feats, img_paths, emotions = batch[count]
                else:
            ...
            for seq_feats in seq_feats_list:
                if config.pool_type == 'mean':
                    ...
                    feats_list.append(attentive_pooling(seq_feats))
