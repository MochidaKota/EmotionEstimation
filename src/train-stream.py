...
def main(config):
    ...
    if use_feat_list['AU'] == 1:
        ...
        emo_net = EmotionEstimator.AUStream(gpool_type=config.gpool_type, dropout=config.dropout)
    if use_feat_list['Gaze'] == 1:
        input_dim = pd.read_pickle(config.gaze_feats_path).shape[1] - 1
        emo_net = EmotionEstimator.GazeStream()
    if use_feat_list['HP'] == 1:
        input_dim = pd.read_pickle(config.hp_feats_path).shape[1] - 1
        emo_net = EmotionEstimator.HPStream()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpool_type', type=str, default='max', choices=['max', 'avg', 'att'], help='global pooling type')
    ...
    parser.add_argument('--check_test_loss', type=str2bool, default=True, help='phase')
